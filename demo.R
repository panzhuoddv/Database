...
lines <- readLines("C:\\pz\\jupyter notebook\\hj\\k.txt")   ##filename.txt是文件名
...
# make Run location dataset
  ...
  #利用b函数，类似于转换字典，用lapply函数转换环境类型为颜色类型list c  然后将点边缘颜色定义为c
  ...
  theme(axis.title.y=element_blank(),
       ...
       guides(size = guide_legend(label.position = "bottom",
                    ...
                    fill = guide_legend(label.position = "bottom",
                                        ...
                                        nrow = 1,
                                        override.aes = list(size=6),#用来设置图例中图形的参数
                                        title = "Environmental type",
  
                                        title.theme = element_text(face = "bold"),#用来设置图例中字体的参数
                                        title.position = "top",#用来设置图例中字体的位置
                                        title.hjust = 0.5))
